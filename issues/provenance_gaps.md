...
- **status:** 사전판정 RESOLVED(naive FAIL/collapsed PASS, Lock 6 유지) — **Phase 7 node-count 실측 완료(100/190 ≪5000)**, dagre ms 라이브 측정만 Phase 8 잔존. [[GAP-33]] 연계.
- **영향 범위:** Phase 8(렌더 — collapse/cull/증분 필수, ms 라이브), ~~Phase 7(decision_tree.json 생성 후 실측)~~ ✅완료. CLAUDE.md DoD #6 정밀화 연계.
...
- **status:** RESOLVED(기록 + slice 6 검증목표 반영). ★ Phase 7 갱신: c0253→INVALID(111)는 결정 B terminal_routing으로 해소. **c0253→Q15D(89)는 결정 A+B 반영 후 유일 잔존 scope-out** — 결정 A(c0252→Q04)와 동형(ROUTE c SSOT가 can_route_to_q 미선언 Q로 라우팅)이나 미승인·cite-verify(universe_sm/q_codes에서 c0253→Q15D 근거) 미완 → **deferred 유지**(다음 spec 결정 대상). [[GAP-15]]/[[GAP-26]]/[[GAP-8]]/[[GAP-34]] 연계.
...
- **status:** 골격(step1~2.5) 완료 + ★ 후속 [[GAP-34]](Phase 7 결정 A·B 반영: c0252/c0204 INFUSION→Q04 spec-change + terminal_routing INVALID). Phase 7 잔여: (1) c0253→Q15D(GAP-28, 유일 잔존 scope-out)·미배선 source c(UNREACHED-Q) → (2) step 2 다발 압축·step 4 orchestrator 재배선(①/C·D·E와 연계).
...
- **★ 잔여(deferred, 사용자 확정):** **c0253→Q15D(89, [[GAP-28]])** = A+B 후 **유일 잔존 scope-out**(결정 A 동형, cite-verify 선행 필요, 미승인) · **UNREACHED-Q Q15A/B/C/X**(source c 미배선 c0368/c0499/c0209; Q15C 단독 비대칭 주입 금지 — 일괄 이월) · [[GAP-13]](c0210 A10)/[[GAP-27]]B/c0040(명시 이월). scope-out 5→**1**/572→**89**.
...
- **status:** ✅ RESOLVED (결정 A·B 반영 완료). [[GAP-5]]/[[GAP-8]]/[[GAP-12]]/[[GAP-31]] RESOLVE 근거. 잔여 [[GAP-28]]/[[GAP-13]]/[[GAP-27]]B/UNREACHED-Q deferred.
