...
- **status:** OPEN (전제 반증 기록 + 작업=27c 신규 구현, 백로그가 추적). **진행: slice 8 Batch A 6 ROUTE 완료(완주 173→353, upstream 6/27 구현), 남은 21 upstream + 46 mess.** measure-not-fix(7b는 구현 0; slice 8은 6c 구현).
- **영향 범위:** Phase 5 이후 column-path 슬라이스 시퀀스(백로그 따라 진행), Phase 5 full-integration 완료 판정(나머지 21 upstream 선결로 **미완**). [[GAP-29]](정규화 = 27c 호출 전 선결, RESOLVED)·[[GAP-31]](slice 8 c0252 divergence) 연계.
...
- **status:** OPEN (postcond-faithful 구현 완료 + divergence 기록·이월). Phase 7 D-S4 / ① 통합 소관.
