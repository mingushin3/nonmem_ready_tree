...
- **status:** 사전판정 RESOLVED(naive FAIL/collapsed PASS, Lock 6 유지) — Phase 8 실데이터 재측정으로 확정 대기.
- **영향 범위:** Phase 8(렌더 — collapse/cull/증분 필수), Phase 7(decision_tree.json 생성 후 실측). CLAUDE.md DoD #6 정밀화 연계.
...
- **status:** RESOLVED(기록 + slice 6 검증목표 반영). c0306/c0253 can_route_to_q의 Q15D/INVALID conditional edge는 Phase 7 D-S4 재구성으로 이월. [[GAP-15]](c0306 producer 종결)/[[GAP-26]](동형 선례)/[[GAP-8]] 연계.
...
- **★ ①/②와 직교(혼동 금지):** 27c는 완주 **경로**만 연다. terminal **실현**(① 외부 meta 주입, GAP-4/6/7/9/10/11/12/14)·tree **구조**(② D-S4 conditional edge, GAP-5/8/12/13)는 27c 후에도 잔존하는 별개 결손. 7b 측정: 완주 strand 기대-q 실현 0(①)·axis 종착 68 terminal=None(②) — 7a와 불변(`test_terminal_realization_still_starved_unchanged`/`test_d_s4_runtime_isolated_still_unchanged`).
- **status:** OPEN (전제 반증 기록 + 작업=27c 신규 구현, 백로그가 추적). measure-not-fix(7b는 구현 0).
- **영향 범위:** Phase 5 이후 column-path 슬라이스 시퀀스(백로그 따라 진행), Phase 5 full-integration 완료 판정(27c 선결로 **미완**). [[GAP-29]](정규화 = 27c 호출 전 선결, RESOLVED) 연계.
