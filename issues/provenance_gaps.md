...
- **권고(미적용, 기록만):** (1) closure_proof.md :74 "tree에서는 live" → "**배선 시** tree-live(현재 미배선→tree 부재)"로 시제 정정. (2) C_dead 처분 결정 시 GAP-30 배선과 함께 일괄. (3) banner 회계에 "orphan C_dead 3" 보조 표기 검토(은폐 아니나 62 외 3 가시화).
- **falsifiable(`tests/test_phase9_adversarial.py` 12 green):** `test_p9_11_c1_dead_c_matches_documented`(dead c == 문서화 C_dead {c0042,c0043,c0333}, **미기록 dead 0** 회귀방지)·`test_p9_2_ds4_wired_no_missing_edge`(wired scope C1/D-S4 무결)·closure_proof.md :103 INV-3·strands_stats.md §6.
- **status:** OPEN (②latent — 즉시 결함 아님, 현 경계의 정직한 미구현; 배선 시 해소). wired scope(57)는 무결(C1 119/122의 3 예외 = 문서화 C_dead 전부). [[GAP-30]](① 배선 백로그)·[[GAP-33]]/[[GAP-34]](Phase 7 골격·결정)·[[GAP-13]]/[[GAP-27]]B(기타 Phase 7 deferred) 연계.
